...
const workspaceRoot = resolve(process.env.BROWSER_SMOKE_WORKSPACE || process.cwd());
const screenshotRoot = join(workspaceRoot, "screenshots");
const outPng = checkedOutputPath(args.outPng, [screenshotRoot]);
const derived = derivedPaths(outPng);
const mobilePng = checkedOutputPath(derived.mobilePng, [screenshotRoot]);
const outJson = checkedOutputPath(derived.verdictJson, [screenshotRoot], "verdict JSON");
...
if (baselineRequested) {
  try {
    baselinePath = checkedOutputPath(realpathSync(args.baseline), [screenshotRoot], "baseline");
  } catch (err) {
...
try {
  ...
  for (const vp of VIEWPORTS) {
    ...
    await page.locator('#main[data-hydrated="true"]').waitFor({ timeout: timeoutMs });

    const title = await page.title();
    const appIdentity = (await page.locator("#main").count()) === 1 && title.includes("Ablit");
    ...
    await page.screenshot({ path: vp.screenshot, fullPage: false, timeout: timeoutMs });
    await page.close();
