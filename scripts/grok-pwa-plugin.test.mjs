...
/** Tests must not leak this workspace's src/lib/og/site.json. */
const ISOLATED_CWD = mkdtempSync(join(tmpdir(), "grok-pwa-isolated-"));

function injectGrokPwaHead(html, ctx = {}) {
  return injectGrokPwaHeadRaw(html, { ...ctx, cwd: ctx.cwd ?? ISOLATED_CWD });
}

function createHeadInjector(ctx = {}) {
  return createHeadInjectorRaw({ ...ctx, cwd: ctx.cwd ?? ISOLATED_CWD });
}
