...
Local **educational workbench** for refusal-suppression research (*abliteration*) — so researchers and defenders can measure the pipeline and **counter illegal misuse**. This is not an attack runbook and not an uncensored-model storefront.
...
PRs welcome. **A contribution is not a commercial license.** Third-party commercial, illegal, and abusive use is **not licensed** — [ACCEPTABLE-USE.en.md](./ACCEPTABLE-USE.en.md).
...
Install, chrome, methods, remaining OPEN: [USER-GUIDE.en.md](./USER-GUIDE.en.md).
...
Completeness is not high yet. Analog web at `:8080` is usable. Packaged Electron, GPU golden, and in-app Hub live are **OPEN**. Send **evidence**, not a flipped markdown cell. analog `npm test` ≠ native close.
