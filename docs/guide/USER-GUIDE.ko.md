...
## 왜 TODO 문서가 공개 트리에 있나

세션 스크래치(`docs/research-*`, CHILD 메모, Vast JSON)는 **올리지 않았습니다.**

올린 것은 코드가 **직접 import/파싱하는 제품 원장**입니다.

| 파일 | 이유 |
| --- | --- |
| `docs/maximum-completion/TODO-ROADMAP.md` | `npm run ledger:status` · 테스트가 01–20 native OPEN을 읽음 |
